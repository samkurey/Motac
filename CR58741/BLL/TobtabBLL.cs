...
namespace TourlistBusinessLayer.BLL
{
    public class TobtabBLL : BusinessLayerBaseClass
    {
        ...
        public List<MM2HBranch> GetMM2HBranchesActive(Guid Org_ref)
        {
            var MM2HBranches = new List<MM2HBranch>();
            var datas = unitOfWork.MM2HAddBranchesRepository.GetMM2HBranchesByOrgID(Org_ref);

            foreach (var data in datas)
            {
                MM2HBranch TTBranch = new MM2HBranch();
                TTBranch.mm2h_add_branches_idx = data.mm2h_add_branches_idx;
                TTBranch.branch_name = data.branch_name;
                TTBranch.stub_ref = data.stub_ref;

                ref_references statusRecord = unitOfWork.RefReferencesRepository.Find(x => x.ref_idx == data.active_status).FirstOrDefault();
                var guidActive = statusRecord != null ? statusRecord.ref_name : "";
                if (guidActive == "ACTIVE") TTBranch.active_status = data.active_status;

                MM2HBranches.Add(TTBranch);
            }
            return MM2HBranches;
        }
        ...
        //added by samsuri (CR#58741) on 24 jan 2024
        private Guid GetMM2HBranchUploadedDoc(string code, Guid stubRef)
        {
            var mm2hLicense = unitOfWork.MM2HLicensesRepository.Find(c => (c.stub_ref == stubRef)).FirstOrDefault();
            var coreChklstLists = unitOfWork.CoreChklstListsRepository.Find(c => (c.chklist_code.ToString() == "MM2H_CHANGESTATUS_DOKUMEN")).FirstOrDefault();
            var coreChkLstItems = unitOfWork.CoreChklstItemsRepository.Find(c => (c.chklist_ref == coreChklstLists.chklist_idx && c.descr_string1 == code)).FirstOrDefault();
            var coreChkitemsInstances = unitOfWork.CoreChkItemsInstancesRepository.Find(c => (c.chklist_instance_ref == mm2hLicense.supporting_document_list && c.chklist_tplt_item_ref == coreChkLstItems.item_idx)).FirstOrDefault();
            if (coreChkitemsInstances == null) return Guid.Empty;
            return coreChkitemsInstances.chkitem_instance_idx;
        }

        //Added by samsuri (CR#57258)  on 10 Jan 2024
        public List<MM2HBranch> GetMM2HBranchesbyBranchIdx(Guid Branch_Idx)
        {
            var mm2hBranches = new List<MM2HBranch>();
            var datas = unitOfWork.MM2HAddBranchesRepository.GetMM2HBranchesByBranchIdx(Branch_Idx);

            foreach (var data in datas)
            {
                MM2HBranch mm2hBranch = new MM2HBranch();
                mm2hBranch.mm2h_add_branches_idx = data.mm2h_add_branches_idx;
                mm2hBranch.branch_name = data.branch_name;
                mm2hBranch.branch_addr_1 = data.branch_addr_1;
                mm2hBranch.branch_addr_2 = data.branch_addr_2;
                mm2hBranch.branch_addr_3 = data.branch_addr_3;
                mm2hBranch.branch_postcode = data.branch_postcode;
                mm2hBranch.branch_phone_no = data.branch_phone_no;
                mm2hBranch.branch_mobile_no = data.branch_mobile_no;
                mm2hBranch.branch_state = data.branch_state != null ? (Guid)data.branch_state : Guid.Empty;
                mm2hBranch.branch_city = data.branch_city != null ? (Guid)data.branch_city : Guid.Empty;
                mm2hBranch.branch_email = data.branch_email;
                mm2hBranch.branch_fax_no = data.branch_fax_no;
                //mm2hBranch.branch_size = data.branch_size;
                mm2hBranch.branch_email = data.branch_email;
                //mm2hBranch.utility = data.utility != null ? (Guid)data.utility : Guid.Empty; //modified b samsuri on 8 jan 2024
                //mm2hBranch.utility_others = data.utility_others;
                mm2hBranch.branch_website = data.branch_website;
                //mm2hBranch.authorized_capital = data.authorized_capital != null ? (Decimal)data.authorized_capital : 0; //modified b samsuri on 8 jan 2024
                //mm2hBranch.paid_capital = data.paid_capital != null ? (Decimal)data.paid_capital : 0; //modified b samsuri on 8 jan 2024
                //var ilpBranchLic = unitOfWork.IlpLicenses.GetIlpLicenseByIdx(data.ilp_license_idx);
                mm2hBranch.stub_ref = data.stub_ref; //use for upload docs
                mm2hBranch.pbt_ref = data.pbt_ref;

                var chkitem_instanceSewaBeliPremis = GetMM2HBranchUploadedDoc("SEWABELI", (Guid)mm2hBranch.stub_ref);//"Perjanjian Sewa Beli Premis"
                ...
                mm2hBranch.fileNameSewaBeliPremis = perjanjianSewaBeliPremis == null ? "" : perjanjianSewaBeliPremis.string1;
                mm2hBranch.fileLocSewaBeliPremis = perjanjianSewaBeliPremis == null ? "" : perjanjianSewaBeliPremis.upload_location;

                var chkitem_instancePelanLantai = GetMM2HBranchUploadedDoc("PELANLANTAI", (Guid)mm2hBranch.stub_ref);//"Pelan Lantai Premis Perniagaan"
                ...
                mm2hBranch.fileNamepelanLantai = pelanLantaiPremisPerniagaan == null ? "" : pelanLantaiPremisPerniagaan.string1;
                mm2hBranch.fileLocpelanLantai = pelanLantaiPremisPerniagaan == null ? "" : pelanLantaiPremisPerniagaan.upload_location;

                var chkitem_instanceGambar = GetMM2HBranchUploadedDoc("GAMBAR", (Guid)mm2hBranch.stub_ref);//"Gambar Bahagian Dalam dan Luar Pejabat (Berwarna)"
                ...
                mm2hBranch.fileNameGambarPermis = gambar == null ? "" : gambar.string1;
                mm2hBranch.fileLocGambarPermis = gambar == null ? "" : gambar.upload_location;

                var datasUpdated = unitOfWork.MM2HAddBranchesRepository.GetMM2HBranchesUpdatedByBranchIdx(Branch_Idx);
                var mm2hBranchUpdated = new List<MM2HBranchUpdated>();
                foreach (var dataUpd in datasUpdated)
                {
                    MM2HBranchUpdated mm2hBranchUpd = new MM2HBranchUpdated();
                    mm2hBranchUpd.new_branch_addr_1 = dataUpd.new_branch_addr_1;
                    mm2hBranchUpd.new_branch_addr_2 = dataUpd.new_branch_addr_2;
                    mm2hBranchUpd.new_branch_addr_3 = dataUpd.new_branch_addr_3;
                    mm2hBranchUpd.new_branch_postcode = dataUpd.new_branch_postcode;
                    mm2hBranchUpd.new_branch_city = (Guid)dataUpd.new_branch_city;
                    mm2hBranchUpd.new_branch_state = (Guid)dataUpd.new_branch_state;

                    mm2hBranchUpd.old_branch_addr_1 = dataUpd.old_branch_addr_1;
                    mm2hBranchUpd.old_branch_addr_2 = dataUpd.old_branch_addr_2;
                    mm2hBranchUpd.old_branch_addr_3 = dataUpd.old_branch_addr_3;
                    mm2hBranchUpd.old_branch_postcode = dataUpd.old_branch_postcode;
                    mm2hBranchUpd.old_branch_city = (Guid)dataUpd.old_branch_city;
                    mm2hBranchUpd.old_branch_state = (Guid)dataUpd.old_branch_state;

                    mm2hBranchUpdated.Add(mm2hBranchUpd);
                }
                mm2hBranch.branch_updated = mm2hBranchUpdated;

                mm2hBranches.Add(mm2hBranch);
            }
            return mm2hBranches;
        }
        ...
        public List<TOBTABBranch> GetTobtabBranchesbyBranchIdx(Guid Branch_Idx)
        {
            ...
            foreach (var data in datas)
            {
                ...
                TTBranch.branch_state = data.branch_state != null ? (Guid)data.branch_state : Guid.Empty;
                TTBranch.branch_city = data.branch_city != null ? (Guid)data.branch_city : Guid.Empty;
...
*/
        ...
        public int CancelBidang(tobtab_licenses tobtab_Licenses, Guid apply_idx, bool outbound, bool inbound, bool ticketing, bool umrah, Guid useridx)
        {
            int ret = 0;

            core_license_updated core_License_Updated = unitOfWork.CoreLicenseUpdatedRepository.Find(x => x.application_stub_ref == apply_idx).FirstOrDefault();
            if (core_License_Updated == null)
            {
                core_License_Updated = new core_license_updated();
                core_License_Updated.license_upd_idx = Guid.NewGuid();
                core_License_Updated.application_stub_ref = apply_idx;
                core_License_Updated.is_corelicensed = 1;
                core_License_Updated.old_inbound = tobtab_Licenses.inbound;
                core_License_Updated.old_outbound = tobtab_Licenses.outbound;
                core_License_Updated.old_ticketing = tobtab_Licenses.ticketing;
                core_License_Updated.old_umrah = tobtab_Licenses.umrah;

                if (inbound)
                    core_License_Updated.new_inbound = 1;
                else
                    core_License_Updated.new_inbound = 0;
                if (outbound)
                    core_License_Updated.new_outbound = 1;
                else
                    core_License_Updated.new_outbound = 0;
                if (ticketing)
                    core_License_Updated.new_ticketing = 1;
                else
                    core_License_Updated.new_ticketing = 0;
                if (umrah)
                    core_License_Updated.new_umrah = 1;
                else
                    core_License_Updated.new_umrah = 0;
                core_License_Updated.active_status = 1;
                core_License_Updated.created_at = DateTime.Now;
                core_License_Updated.modified_at = core_License_Updated.created_at;
                core_License_Updated.created_by = useridx;
                core_License_Updated.modified_by = useridx;
                unitOfWork.CoreLicenseUpdatedRepository.Add(core_License_Updated);
            }
            else
            {
                core_License_Updated.old_inbound = tobtab_Licenses.inbound;
                ...
                if (umrah)
                    ...
                    core_License_Updated.new_umrah = 0;
                core_License_Updated.active_status = 1;
                core_License_Updated.modified_at = DateTime.Now;
                core_License_Updated.modified_by = useridx;
                unitOfWork.CoreLicenseUpdatedRepository.Update(core_License_Updated);
            }

            ret = unitOfWork.Complete();
